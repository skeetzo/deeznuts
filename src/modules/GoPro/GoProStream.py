...
def gopro_live():
	...
	if "HD4" in response or "HD3.2" in response or "HD5" in response or "HX" in response or "HD6" in response:
		...
		if SAVE == False:
			...
			subprocess.Popen("ffmpeg -re -i 'udp://10.5.5.100:8554' -loglevel "+LOGLEVEL+" -movflags faststart -probesize 100M -analyzeduration 100M -crf 20 -preset slow -f:v mpegts -acodec copy -vcodec copy -f flv rtmp://alexdeeznuts.com:1935/"+DESTINATION, shell=True)
