...
def gopro_live():
	...
	if "HD4" in response or "HD3.2" in response or "HD5" in response or "HX" in response or "HD6" in response:
		...
		if SAVE == False:
			...
			subprocess.Popen("ffmpeg -re -i 'udp://10.5.5.100:8554' -loglevel debug -probesize 100M -analyzeduration 100M -fflags nobuffer -f:v mpegts -vcodec libx264 -preset fast -acodec aac rtmp://192.168.1.10:8935/live/stream?sign=1542970244462-8185f8222a7e830c60f29ae1eaa53161", shell=True)
		#	subprocess.Popen("ffmpeg -i 'udp://:10.5.5.100:8554' -fflags nobuffer -f:v mpegts -probesize 8192 " + TS_PARAMS + SAVELOCATION, shell=True)
